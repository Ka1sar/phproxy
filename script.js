...
function str_rot13(str) {
 ...
 for(var i = 0; i < str.length; i++) {
  ...
  if(curLet == '#') window.document.getElementById('proxy_form').action += str.substring(i, str.length)
...
function submit_form() {
 ...
 for(i = 0; i < document.forms[proxy_settings_form_name].elements['ops[]'].length; i++) flags += (document.forms[proxy_settings_form_name].elements['ops[]'][i].checked == true) ? '1' : '0';
 ...
 if(rotate13) url = str_rot13(url);
 else if(base64) url = base64_encode(url);
