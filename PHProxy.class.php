<?php
class PHProxy {
 var $banned_hosts = array('.localhost','127.0.0.1');
 var $flags = array (
  'include_form'    => 1, 
  'remove_scripts'  => 1,
  'accept_cookies'  => 1,
 ...
 function PHProxy($config, $flags = 'previous') {
  ...
  if($this->flags['rotate13']) {
   ...
   function decode_url($url) {return str_replace('&amp;','&',str_rot13(rawurldecode($url)));}
  }
  else if($this->flags['base64_encode']) {
   function encode_url($url) {return rawurlencode(base64_encode($url));}
   function decode_url($url) {return str_replace('&amp;','&',base64_decode(rawurldecode($url)));}
  }
  else {
   function encode_url($url) {return rawurlencode($url);}
   function decode_url($url) {return str_replace('&amp;','&',rawurldecode($url));}
  }
 }
 ...
 function open_socket() {
  ...
  if($this->socket === false) $this->trigger_error("$err_no: $err_str (URL: {$this->url_segments['host']})", encode_url($this->url));
 }
 ...
 function set_response() {
  ...
  if($this->flags['accept_cookies'] == 1) $this->set_cookies();
 ...
 function set_content_length() {$this->content_length = (preg_match("#content-length:([^\r\n]*)#i", $this->response_headers, $matches) && trim($matches[1]) != '') ? trim($matches[1]) : false;}
 ...
 function set_url($url) {
  ...
  if(strpos($this->url, '://') === false) $this->url = 'http://' . $this->url;
  if($this->parse_url($this->url, $this->url_segments)) {
   $this->base = $this->url_segments;
   if(!$this->is_allowed_host()) exit();
  }
  else $this->trigger_error('Please supply a valid URL');
 }
 ...
 function is_allowed_host() {
  if(!empty($this->banned_hosts)) {
   foreach ($this->banned_hosts as $host) {
	if (($host{0} == '.' && preg_match('#' . trim($host, '.') . '$#i', $this->url_segments['host'])) || strcasecmp($host, $this->url_segments['host']) == 0) return false;
   }
   return true;
  }
  return true;
 ...
 function modify_urls() {
  ...
  for($i = 0, $count_i = count($matches); $i < $count_i; $i++) $this->response_body = str_replace($matches[$i][0], $matches[$i][1]. $this->proxify_css($matches[$i][2]) .$matches[$i][3], $this->response_body);
  preg_match_all("#<\s*([a-zA-Z]+)([^>]+)>#", $this->response_body, $matches);
  for($i = 0, $count_i = count($matches[0]); $i < $count_i; $i++) {
   $tag = strtolower($matches[1][$i]);
   if(!isset($tags[$tag]) || !preg_match_all("#([a-zA-Z\-\/]+)\s*(?:=\s*(?:\"([^\">]*)\"?|'([^'>]*)'?|([^'\"\s]*)))?#", $matches[2][$i], $m, PREG_SET_ORDER)) continue;
   $rebuild    = false;
   $extra_html = $temp = '';
   $attrs      = array();
   ...
   case 'meta':
    if($this->flags['strip_meta'] && isset($attrs['name']) && preg_match('#(keywords|description)#i', $attrs['name'])) $this->response_body = str_replace($matches[0][$i], '', $this->response_body);
 ...
 function proxify_css($css) {
  ...
  for($i = 0, $count = count($matches); $i < $count; $i++) $css = str_replace($matches[$i][0], 'url(' . $this->proxify_css_url($matches[$i][1]) . ')', $css);
 ...
 function set_flags($flags) {
  ...
  else if(isset($_COOKIE['flags']) && is_numeric($_COOKIE['flags']) && strlen($_COOKIE['flags']) == count($this->flags)) $this->set_flags($_COOKIE['flags']);
  else {
   $flags = '';
   foreach($this->flags as $flag) $flags .= $flag;
   $this->set_flags($flags);
  }
 }
 ...
 function set_request_headers() {
  ...
  else $headers .= "Accept: */*;q=0.1\r\n";
  if($this->flags['show_referer'] == 1) $headers .= "Referer: {$this->url_segments['base']}\r\n";
  if(($cookies = $this->get_cookies('COOKIE')) != '') $headers .= "Cookie: $cookies\r\n";
  if(!empty($this->basic_auth_header)) $headers .= "Authorization: Basic {$this->basic_auth_header}\r\n";
 ...
 function set_post_vars($array, $parent_key = null) {
  ...
  foreach($array as $key => $value) {
   ...
   if (is_array($value)) $tmp = array_merge($tmp, $this->set_post_vars($value, $key));
   else $tmp[$key] = urlencode($value);
  }
  return $tmp;
 }
 ...
 function set_post_files($array, $parent_key = null) {
  ...
  foreach($array as $key => $value) {
   $key = isset($parent_key) ? sprintf('%s[%s]', $parent_key, urlencode($key)) : urlencode($key);
   if(is_array($value)) $tmp = array_merge_recursive($tmp, $this->set_post_files($value, $key));
   else if(preg_match('#^([^\[\]]+)\[(name|type|tmp_name)\]#', $key, $m)) $tmp[str_replace($m[0], $m[1], $key)][$m[2]] = $value;
  }
  return $tmp;
 }
 ...
 function follow_location() {
  if(preg_match("#(location|uri|content-location):([^\r\n]*)#i", $this->response_headers, $matches)) {
   if(($url = trim($matches[2])) == '') return false;
 ...
 function set_cookies() {
  ...
  foreach($matches[1] as $cookie_info) {
   ...
   $domain  = rtrim($domain, '.');
   if(!preg_match("#$domain$#i", $this->url_segments['host'])) continue;
   if(count($_COOKIE) >= 15 && time()-$expires < 0) setcookie(current($_COOKIE), '', 1, '', $this->http_host);
 ...
 function get_cookies($type = 'COOKIE', $restrict = true) {
  if(!empty($_COOKIE)) {
   ...
   foreach($_COOKIE as $cookie_name => $cookie_value) {
    ...
    if($cookie_args[0] != $type || count($cookie_args) < 2) continue;
    if($type == 'AUTH') {
     if($this->url_segments['host'] == str_replace('_', '.', $cookie_args[1])) return $cookie_value;
     else continue;
	}
 ...
 function delete_cookies($hash) {
  ...
  foreach($cookies as $args) {
   if ($hash == 'all' || $hash == md5($args[0].$args[1].$args[2].$args[3])) setcookie(urlencode("COOKIE;$args[0];$args[1];$args[2]"), '', 1, '', $this->http_host);
  }
 }
 ...
 function send_response_headers() {
  ...
  if (!empty($this->response_body)) $headers[] = 'Content-Length: ' . strlen($this->response_body);
  $headers = array_filter($headers);
  foreach ($headers as $header) header($header);
 }

 function return_response($send_headers = true) {
  if ($this->content_type == 'text/css') $this->response_body = $this->proxify_css($this->response_body);
  else {
   if($this->flags['strip_title']) $this->response_body = preg_replace('#(<\s*title[^>]*>)(.*?)(<\s*/title[^>]*>)#is', '$1$3', $this->response_body);
   if($this->flags['remove_scripts']) $this->remove_scripts();
   if($this->flags['show_images'] == 0) $this->response_body = preg_replace('#<(img|image)[^>]*?>#si', '', $this->response_body);
   $this->modify_urls();
  }
  if ($send_headers) $this->send_response_headers();
 ...
 function options_list($tabulate = false, $comments_on = false) {
  ...
  foreach($this->flags as $flag_code => $flag_status) {
   ...
   if(!$tabulate) $interface = array_reverse($interface);
