...
if(isset($_GET[$config['flags_var_name']])) $flags = $_GET[$config['flags_var_name']];
...
if(isset($_GET[$PHProxy->config['get_form_name']])) {
 ...
 if (preg_match('#^'.$PHProxy->config['get_form_name'].'#', $arr[0])) array_shift($arr);
...
<html>
<head>
<title>pleb.x10.mx | PHProxy</title>
...
if(isset($_GET['error'])) {
 ...
 if(isset($_GET['retry'])) echo '<div class="error"><a href="'. $PHProxy->proxify_url(decode_url($_GET['retry'])) .'">Retry</a></div>';
}
...
if(isset($_GET['action'])) {
 if($_GET['action'] == 'cookies') {
  ...
  else echo '<div class="error">No cookies available.</div>';
 }
